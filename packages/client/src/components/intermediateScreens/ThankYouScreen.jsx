...
function ThankYouScreen({ founder = false, onFinished = () => {} } ) {
  ...
  // start animations when all images are loaded and the image list length is set
  useEffect(() => {
    // select a subset of 10 random images
    if (founder && founderListLength) {
      Animated.parallel([
        // make the founder images appear
        Animated.timing(founderOpacity, {
          toValue: 1,
          duration: fadeOutTime,
          useNativeDriver: true,
        }),
        // animate the founder images scrolling
        Animated.timing(founderTranslateX, {
          // translate to the length of the image array,
          // minus an ammount to make it stop at the last image
          toValue: founderInitialTranslateX - (founderListLength - 40),
          duration: 6000,
          useNativeDriver: true,
        }),
      ]).start(() => {
        // when the animation is finished, call the onFinished callback
        onFinished();
      });
      // animate the loading bar
      Animated.loop(
        Animated.sequence([
          Animated.timing(loadingTop, {
            toValue: 200,
            duration: 1200,
