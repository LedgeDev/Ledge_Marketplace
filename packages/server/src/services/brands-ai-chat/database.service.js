...
async function generateDatabaseQuery(consult, schemaContent, brandId, retryCount = 0) {
  try {
...
2. Doesnt give away information abaout any specific brand that its not ${brandId}
...
3. Only use attributes of the models that exist in the schema
...
async function handleDatabaseQuery(consult, schemaContent, brandId, previousMessages = [], retryCount = 0) {
  try {

    console.log('Handling database query:', consult);

    // Generate query from user's input
    const query = await generateDatabaseQuery(consult, schemaContent, brandId);
    ...
    if (retryCount < MAX_RETRIES) {
      await new Promise(resolve => setTimeout(resolve, 1000 * retryCount));
      return handleDatabaseQuery(consult, schemaContent, brandId, previousMessages, retryCount + 1);
    }
    throw error;
  }
}
...
async function handleSearchQuery(searchQuery, schemaContent, brandId, retryCount = 0) {
  try {
    // Get base query for entity
    const {
      query,
      entityName
    } = await identifyEntityFromSchema(searchQuery, schemaContent, brandId);
    ...
    return 'I found some potential matches, but none of them seem to match what you are looking for :(. Could you provide more details or try rephrasing your query?';

  } catch (error) {
    console.error('Search query failed:', error);
    if (retryCount < MAX_RETRIES) {
      await new Promise(resolve => setTimeout(resolve, 1000 * (retryCount + 1)));
      return handleSearchQuery(searchQuery, schemaContent, brandId, retryCount + 1);
    }
